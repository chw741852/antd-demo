/**
 * Created by caihongwei on 19/04/2017.
 */
        ...
        return (
            <form onSubmit={event => {
                ...
                <Prompt when={isBlocking} message={location => (
                    `你真的要调到${location.pathname}么？`
                )}/>

                <p>
                    是否无法跳转？{isBlocking ? '好，现在试试再点击那些链接' : '可以正常跳转'}
                </p>
                <p>
                    <input size="50" placeholder="你这里面输入了以后就不能正常跳转了" onChange={
                        event => {
                            this.setState({
